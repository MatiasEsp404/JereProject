...
public class GetAuthenticatedUserDetailsIntegrationTest extends BigTest {
	...
	public void shouldReturnUserWhenHasUserRole() throws Exception {
		...
		mockMvc.perform(get("/auth/me").contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION,
				getAuthorizationTokenForStandardUser())).andExpect(jsonPath("$.firstName", equalTo("Freddy")))
	...
	public void shouldReturnUserWhenHasAdminRole() throws Exception {
		...
		mockMvc.perform(get("/auth/me").contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION,
				getAuthorizationTokenForAdminUser())).andExpect(jsonPath("$.firstName", equalTo("Jason")))
